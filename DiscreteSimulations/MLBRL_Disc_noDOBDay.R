...
########################################################################################################################
########Implements MLBRL for the Simulation Setting with Error in Region, Error in Area Income, Error in Day of Birth
########################################################################################################################

nNumReplications = 100
...
if(file.exists(sprintf("Sensitivity_Joint_MLBRL_R%d_I%d_D%d_noD.csv", Regionerrorprob*10, IncomeError*10, DOBerrorprob*10)))
{
  ...
  Sensitivity=rep(0,nNumReplications); Sensitivity_sd=rep(0,nNumReplications)
  PPV=rep(0,nNumReplications); PPV_sd=rep(0,nNumReplications)
  F1=rep(0,nNumReplications); F1_sd=rep(0,nNumReplications)
  Accuracy=rep(0,nNumReplications); Accuracy_sd=rep(0,nNumReplications)
}
...
for(nrep in nStartVal:nNumReplications){
  set.seed(seed[nrep])
  ############################################################
  ...
  IncomeP_gamma=as.vector(apply(IncomeP_comparison,2,function(x,Income_AP) abs(x-Income_AP)<=500,N_A$Income)*2)
  ...
  g8 <- gammaNUMCK2par(N_A$Income, N_B$Income, cut.a=500)
  ...
  prior_BM_Region=c(1,1); prior_BM_Status=c(1,1); prior_BM_Type=c(1,1); prior_BM_SES=c(1,1); prior_BM_Income=c(1,1)
  prior_BU_Region=c(1,1); prior_BU_Status=c(1,1); prior_BU_Type=c(1,1); prior_BU_SES=c(1,1); prior_BU_Income=c(1,1)
  ...
  theta_BM_Income=matrix(0,nrow=length(prior_BM_Income),ncol=nsim)
  ...
  theta_BU_Income=matrix(0,nrow=length(prior_BU_Income),ncol=nsim)
  ...
  #Export Designations of Linking Matrix after every iteration of updating the linking matrices
    ...
    ##################################################################################################
    # write.csv(theta_BM_Region,paste0("theta_BM_Region_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BM_Status,paste0("theta_BM_Status_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BM_Type,paste0("theta_BM_Type_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BM_Income,paste0("theta_BM_Income_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BM_SES,paste0("theta_BM_SES_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BU_Region,paste0("theta_BU_Region_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BU_Status,paste0("theta_BU_Status_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BU_Type,paste0("theta_BU_Type_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BU_Income,paste0("theta_BU_Income_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_BU_SES,paste0("theta_BU_SES_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CM_DOB,paste0("theta_CM_DOB_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CM_Gender,paste0("theta_CM_Gender_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CU_DOB,paste0("theta_CU_DOB_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CU_Gender,paste0("theta_CU_Gender_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CBU_DOB,paste0("theta_CBU_DOB_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(theta_CBU_Gender,paste0("theta_CBU_Gender_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(BlockDesignation,paste0("BlockDesignation_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
    # write.csv(LinkDesignation,paste0("LinkDesignation_MLBRL_R0_I0_D0_noD_",nrep,".csv"))
  }
