...
for(nrep in nStartVal:100){
  ...
  #Income_Diff_flat=as.vector(Income_Diff[c(rep(1:nrow(Income_Diff), each=15), rep(1:nrow(Income_Diff), each=5)), c(rep(1:ncol(Income_Diff), each=15), rep(1:ncol(Income_Diff), each=15))])
  ...
  #                         Zeta6_flat,Zeta7_flat,Zeta8_flat,Income_Diff_flat)
  ...
  #GammaZetaIDs=data.frame(A_LinkID,B_LinkID,Full_BlockID_A,Full_BlockID_B)
  #GammaZeta=(cbind(Zeta1_flat,Zeta2_flat,Zeta3_flat,Zeta4_flat,Zeta5_flat,
  #                          Zeta6_flat,Zeta7_flat,Zeta8_flat,Income_Diff_flat))
  
  numContVar = 1
  #nsim_EM=100
  #Initialize parameters for EM algorithm
  #theta_M=matrix(0,ncol=dim(GammaZeta)[2],nrow=nsim_EM)
  #theta_M=matrix(0,ncol=dim(GammaZeta)[2]+numContVar,nrow=nsim_EM)
  #theta_U=matrix(0,ncol=dim(GammaZeta)[2],nrow=nsim_EM)
  #theta_U=matrix(0,ncol=dim(GammaZeta)[2]+numContVar,nrow=nsim_EM)
  #pi_M=matrix(0,ncol=1,nrow=nsim_EM)
  #loglik=rep(0,length=nsim_EM)
  
  #Set starting values for EM algorithm parameters
  #theta_M[1,]=c(.05,.25,.7,.25,.75,.3,.7,.25,.75,.4,.6,.35,.65,.2,.8)
  #theta_M[1,]=c(.05,.25,.7,.25,.75,.3,.7,.25,.75,.4,.6,.35,.65, 0, 5^2)
  #theta_U[1,]=c(.7,.20,.10,.75,.25,.66,.34,.75,.25,.6,.4,.65,.35,.8,.2)
  #theta_U[1,]=c(.7,.20,.10,.75,.25,.66,.34,.75,.25,.6,.4,.65,.35,0,10^2)
  #pi_M[1]=runif(1,0,1)
  
  #EM algorithm for probabilistic record linkage
  # for (t in 2:nsim_EM){
  #   product1=t(theta_M[t-1,1:(dim(theta_M)[2]-numContVar*2)]^t(GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)]))
  #   product1= cbind(product1,dnorm(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]],mean= theta_M[t-1,seq(dim(theta_M)[2]-numContVar*2+1,dim(theta_M)[2],2)], sqrt(theta_M[t-1,seq(dim(theta_M)[2]-numContVar*2+2,dim(theta_M)[2],2)])))
  #   product2=t(theta_U[t-1,1:(dim(theta_U)[2]-numContVar*2)]^t(GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)]))
  #   product2= cbind(product2,dnorm(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]], mean= theta_U[t-1,seq(dim(theta_U)[2]-numContVar*2+1,dim(theta_U)[2],2)], sqrt(theta_U[t-1,seq(dim(theta_U)[2]-numContVar*2+2,dim(theta_U)[2],2)])))
  #   
  #   prob=(pi_M[t-1]*apply(product1,1,prod))/((pi_M[t-1]*apply(product1,1,prod))+((1-(pi_M[t-1]))*apply(product2,1,prod)))
  # 
  #   pi_M[t]=sum(prob)/length(prob)
  #   meanM = apply(prob*as.matrix(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]]),2,sum)/sum(prob)
  #   varM = (apply(prob*(as.matrix((GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]]-meanM)^2)),2,sum) + 5^2)/(1+3+ sum(prob))
  #   theta_M[t,]=c((apply(prob*GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)],2,sum)+0.01)/(1+sum(prob)), 
  #                 meanM, varM)
  #   meanU = apply((1-prob)*as.matrix(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]]),2,sum)/sum(1-prob)
  #   varU = (apply((1-prob)*(as.matrix(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]]-meanM)^2),2,sum)+5^2)/(1+3+sum(1-prob))
  #   theta_U[t,]=c((apply((1-prob)*GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)],2,sum)+0.01)/(1+sum(1-prob)),
  #                 meanU,varU)
  #   
  #   #loglik[t]=sum(prob)*log(pi_M[t])+sum(apply(prob*GammaZeta[,theta_M[t,]>0],2,sum)*log(theta_M[t,theta_M[t,]>0]))+
  #   #          sum(1-prob)*log(1-pi_M[t])+sum(apply((1-prob)*GammaZeta[,theta_U[t,]>0],2,sum)*log(theta_U[t,theta_U[t,]>0]))
  #   product1l=t(theta_M[t,1:(dim(theta_M)[2]-numContVar*2)]^t(GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)]))
  #   product1l= cbind(product1,dnorm(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]],mean= theta_M[t,seq(dim(theta_M)[2]-numContVar*2+1,dim(theta_M)[2],2)], sqrt(theta_M[t,seq(dim(theta_M)[2]-numContVar*2+2,dim(theta_M)[2],2)])))
  #   product2l=t(theta_U[t,1:(dim(theta_U)[2]-numContVar*2)]^t(GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)]))
  #   product2l= cbind(product2,dnorm(GammaZeta[,(dim(GammaZeta)[2]-numContVar+1):dim(GammaZeta)[2]], mean= theta_U[t,seq(dim(theta_U)[2]-numContVar*2+1,dim(theta_U)[2],2)], sqrt(theta_U[t,seq(dim(theta_U)[2]-numContVar*2+2,dim(theta_U)[2],2)])))
  #   loglik[t]=sum(log(pi_M[t]*apply(product1l,1,prod) + (1-pi_M[t])*apply(product2l,1,prod)))
  #   if(abs(loglik[t]-loglik[t-1])<.00001){break}
  # }
  
  #Calculate the linkage weights as the ratio of the M and U probabilities for each record pair
  #mprod=t(theta_M[t,1:(dim(theta_M)[2]-numContVar*2)]^t(GammaZeta[,1:(dim(GammaZeta)[2]-numContVar)]))
  #m=pi_M[t]*apply(mprod,1,prod)
  #m=pi_M[t]*apply(product1l,1,prod)
  
  #uprod=t(theta_U[t,theta_U[t,]>0]^t(GammaZeta[,theta_U[t,]>0]))
  #u=(1-pi_M[t])*apply(uprod,1,prod)
  #u=(1-pi_M[t])*apply(product2l,1,prod)
  ...
  #block_projection=function(B, B_S, B_T){
  #  ST=which(B==1,arr.ind=T)
  #  projection=matrix(0,nrow=nrow(B_S),ncol=ncol(B_T))
  #  for (i in 1:dim(ST)[1]){
  #    block_ind_S=B_S==ST[i,1]
  #    block_ind_T=B_T==ST[i,2]
  #    block_ind=block_ind_S*block_ind_T
  ...
  for(t in 2:nsim){
    ...
    for(s in 1:dim(b)[1]){
      ...
      #C[ST_ind==1]=matrix(C_ST, numRowUse, numColUse)
    ...
    for(s in 1:dim(b)[1]){
      ...
      if(sum(b[,2]==r)==0){
        ...
        if(log(runif(1))<acceptance){
          ...
          #C[ST_ind==1]=0; C[SR_ind==1]=C_ST; C[QR_ind==1]=0; C[QT_ind==1]=C_QT
