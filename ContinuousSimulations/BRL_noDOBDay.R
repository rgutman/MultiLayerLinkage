...
########################################################################################################################
########Implements BRL for the Simulation Setting with 20% Error in Region, 20% Error in Area Income, 20% Error in Day of Birth
########################################################################################################################
...
theta_CM_Income=matrix(0,nrow=length(prior_CM_Income)/2,ncol=nsim)
theta_CU_Income=matrix(0,nrow=length(prior_CU_Income)/2,ncol=nsim)
...
tempLink = fastLink(N_A,N_B,varnames=c("Gender","DOB_year","DOB_month","Region","Status","Type","SES","Income"), 
                    numeric.match = c("Income"))
C[as.matrix(tempLink$matches)] = 1
...
theta_CM_Income[,1]=c(0,5^2)
theta_CU_Income[,1]=c(0,10^2)
...
for(t in 2:nsim){
  ...
  n_M = sum(C)
  v_nM = prior_CM_Income[3] + n_M 
  sig_nM = (prior_CM_Income[3]*prior_CM_Income[4] + (n_M - 1)*var(Income_Diff[C==1]) + 
              prior_CM_Income[2]*n_M/(prior_CM_Income[2] + n_M)*(mean(Income_Diff[C==1])- prior_CM_Income[1])^2)/v_nM
  mu_nM = prior_CM_Income[2]/(prior_CM_Income[2] + n_M)*prior_CM_Income[1] + n_M/(prior_CM_Income[2] + n_M)*mean(Income_Diff[C==1])
  k_nM = prior_CM_Income[2]+n_M
  theta_CM_Income[2,t]=rinvgamma(1,v_nM/2,v_nM*sig_nM/2)
  theta_CM_Income[1,t]=rnorm(1,mean=mu_nM, sqrt(theta_CM_Income[2,t]/k_nM))
  
  
  n_U = sum(1-C)
  v_nU = prior_CU_Income[3] + n_U 
  sig_nU = (prior_CU_Income[3]*prior_CU_Income[4] + (n_U - 1)*var(Income_Diff[C==0]) + 
              prior_CU_Income[2]*n_U/(prior_CU_Income[2] + n_U)*(mean(Income_Diff[C==0])- prior_CU_Income[1])^2)/v_nU
  mu_nU = prior_CU_Income[2]/(prior_CU_Income[2] + n_U)*prior_CU_Income[1] + n_U/(prior_CU_Income[2] + n_U)*mean(Income_Diff[C==0])
  k_nU = prior_CU_Income[2]+n_U
  theta_CU_Income[2,t]=rinvgamma(1,v_nU/2,v_nU*sig_nU/2)
  theta_CU_Income[1,t]=rnorm(1,mean=mu_nU, sqrt(theta_CU_Income[2,t]/k_nU))
  ...
  for(i in 1:nrow(C)){
    ...
    Zeta_Income=Income_Diff[i,B_unlinked]
    ...
    num=apply(theta_CM_DOB[,t]^t(Gamma_DOB),2,prod)*apply(theta_CM_Gender[,t]^t(Gamma_Gender),2,prod)*
      ...
      (dnorm(Zeta_Income,mean=theta_CM_Income[1,t],sqrt(theta_CM_Income[2,t])))
    ...
    den=apply(theta_CU_DOB[,t]^t(Gamma_DOB),2,prod)*apply(theta_CU_Gender[,t]^t(Gamma_Gender),2,prod)*
      ...
      (dnorm(Zeta_Income,mean=theta_CU_Income[1,t],sqrt(theta_CU_Income[2,t])))
    Likelihood=num/den
  ...
  # write.csv(theta_CM_Region,paste0("theta_BM_Region_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_Status,paste0("theta_BM_Status_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_Type,paste0("theta_BM_Type_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_SES,paste0("theta_BM_SES_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_Income,paste0("theta_BM_Income_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_Region,paste0("theta_BU_Region_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_Status,paste0("theta_BU_Status_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_Type,paste0("theta_BU_Type_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_SES,paste0("theta_BU_SES_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_Income,paste0("theta_BU_Income_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_DOB,paste0("theta_CM_DOB_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CM_Gender,paste0("theta_CM_Gender_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_DOB,paste0("theta_CU_DOB_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(theta_CU_Gender,paste0("theta_CU_Gender_BRL_R2_I2_D2_noD_",nrep,".csv"))
  # write.csv(LinkDesignation,paste0("LinkDesignation_BRL_R2_I2_D2_noD_",nrep,".csv"))
  print(paste("Iter",t))
}
